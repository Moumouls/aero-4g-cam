...
# DEV mode has VERBOSE enabled by default (can be overridden with VERBOSE=false)
...
# Show startup message in verbose mode
if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
    echo "🚀 Starting DEV mode (with emulator display)..."
    echo ""
fi
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    log_warn "⚠️  Unknown OS: $OSTYPE. Using default ANDROID_HOME"
    ANDROID_HOME="${ANDROID_HOME:-$HOME/Android/sdk}"
    SYSTEM_IMAGE_ARCH="x86_64"
fi
...
# 2. Check if emulator AVD exists and recreate with Pixel 2
...
if "$ANDROID_HOME/cmdline-tools/latest/bin/avdmanager" list avd 2>/dev/null | grep -q "test_emulator"; then
    log_info "Removing existing emulator AVD to recreate with Pixel 2..."
    if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
...
log_info "Creating emulator AVD for $SYSTEM_IMAGE_ARCH with Pixel 2..."
if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
...
log_success "✅ Emulator AVD ready (Pixel 2)"
[ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ] && echo ""

# 3. Start emulator WITH window (for dev mode)
log_info "📱 Step 3/4: Starting emulator with display..."
# Kill any existing emulator
"$ANDROID_HOME/platform-tools/adb" emu kill 2>/dev/null || true
pkill -f "emulator.*test_emulator" 2>/dev/null || true
sleep 2

# Start emulator with window (Linux might need additional flags)
if [[ "$OS_TYPE" == "linux" ]]; then
    # Linux: Check if we have GPU acceleration
    if [ -e /dev/kvm ]; then
        if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
            "$ANDROID_HOME/emulator/emulator" -avd test_emulator -no-snapshot-load -no-audio -accel on &
        else
            "$ANDROID_HOME/emulator/emulator" -avd test_emulator -no-snapshot-load -no-audio -accel on > /dev/null 2>&1 &
        fi
    else
        log_warn "⚠️  KVM not available, emulator will be slow"
        if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
            "$ANDROID_HOME/emulator/emulator" -avd test_emulator -no-snapshot-load -no-audio &
        else
            "$ANDROID_HOME/emulator/emulator" -avd test_emulator -no-snapshot-load -no-audio > /dev/null 2>&1 &
        fi
    fi
else
    # macOS
...
[ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ] && echo ""
node src/automation/camera-recorder.js --local

[ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ] && echo ""
log_success "🎉 Dev session completed successfully!"
