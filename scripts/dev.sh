...
# 2. Check if emulator AVD exists and recreate with Pixel 5
...
if "$ANDROID_HOME/cmdline-tools/latest/bin/avdmanager" list avd 2>/dev/null | grep -q "test_emulator"; then
    log_info "Removing existing emulator AVD to recreate with Pixel 5..."
    if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
...
log_info "Creating emulator AVD for $SYSTEM_IMAGE_ARCH with Pixel 5..."
if [ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ]; then
...
log_success "✅ Emulator AVD ready (Pixel 5)"
[ "$VERBOSE" = "true" ] || [ "$VERBOSE" = "1" ] && echo ""
